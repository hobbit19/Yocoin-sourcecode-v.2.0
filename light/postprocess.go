...
func NewChtIndexer(db yocdb.Database, clientMode bool) *core.ChainIndexer {
	...
	backend := &ChtIndexerBackend{
		diskdb:      db,
		triedb:      trie.NewDatabase(yocdb.NewTable(db, ChtTablePrefix)),
...
const (
	...
	yocBloomBitsSection       = 4096
	yocBloomBitsConfirmations = 256
)
...
func NewBloomTrieIndexer(db yocdb.Database, clientMode bool) *core.ChainIndexer {
	backend := &BloomTrieIndexerBackend{
		diskdb: db,
		triedb: trie.NewDatabase(yocdb.NewTable(db, BloomTrieTablePrefix)),
	}
	...
	if clientMode {
		...
		backend.parentSectionSize = yocBloomBitsSection
		confirmReq = HelperTrieProcessConfirmations
	}
	backend.bloomTrieRatio = BloomTrieFrequency / backend.parentSectionSize
	backend.sectionHeads = make([]common.Hash, backend.bloomTrieRatio)
	return core.NewChainIndexer(db, idb, backend, BloomTrieFrequency, confirmReq-yocBloomBitsConfirmations, time.Millisecond*100, "bloomtrie")
}
