...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute YoCoin requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var yoc = web3.yoc; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = yoc.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines yoc.getBalance(yoc.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Yocoin instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Yocoin metadata
	fmt.Fprintf(c.printer, "Welcome to the YoCoin JavaScript console!\n\n")
	c.jsre.Run(`
