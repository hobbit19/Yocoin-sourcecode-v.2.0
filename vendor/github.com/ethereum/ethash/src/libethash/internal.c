...
// SeqMemoHash(s, R, N)
bool static yochash_compute_cache_nodes(
	node* const nodes,
	uint64_t cache_size,
	yochash_h256_t const* seed
)
{
...
)
{
	...
	for (uint32_t n = 0; n != max_n; ++n) {
		...
		yochash_calculate_dag_item(&(full_nodes[n]), n, light);
	}
	return true;
}

static bool yochash_hash(
	yochash_return_value_t* ret,
	...
	for (unsigned i = 0; i != ETHASH_ACCESSES; ++i) {
		...
		for (unsigned n = 0; n != MIX_NODES; ++n) {
			...
			if (full_nodes) {
				...
				node tmp_node;
				yochash_calculate_dag_item(&tmp_node, index * MIX_NODES + n, light);
				dag_node = &tmp_node;
			}
...
)
{
	...
	yochash_h256_t return_hash;
	yochash_quick_hash(&return_hash, header_hash, nonce, mix_hash);
	return yochash_check_difficulty(&return_hash, boundary);
}

yochash_light_t yochash_light_new_internal(uint64_t cache_size, yochash_h256_t const* seed)
{
	struct yochash_light *ret;
	...
	if (!yochash_compute_cache_nodes(nodes, cache_size, seed)) {
...
{
	yochash_h256_t seedhash = yochash_get_seedhash(block_number);
	yochash_light_t ret;
	ret = yochash_light_new_internal(yochash_get_cachesize(block_number), &seedhash);
...
)
{
  	yochash_return_value_t ret;
	ret.success = true;
	if (!yochash_hash(&ret, NULL, light, full_size, header_hash, nonce)) {
		ret.success = false;
	}
	return ret;
}
...
{
	uint64_t full_size = yochash_get_datasize(light->block_number);
	return yochash_light_compute_internal(light, full_size, header_hash, nonce);
}

static bool yochash_mmap(struct yochash_full* ret, FILE* f)
{
	int fd;
	char* mmapped_data;
	errno = 0;
	ret->file = f;
	if ((fd = yochash_fileno(ret->file)) == -1) {
		return false;
	}
	mmapped_data= mmap(
		NULL,
...
)
{
	...
	switch (yochash_io_prepare(dirname, seed_hash, &f, (size_t)full_size, false)) {
	case ETHASH_IO_FAIL:
		// yochash_io_prepare will do all ETHASH_CRITICAL() logging in fail case
		goto fail_free_full;
	case ETHASH_IO_MEMO_MATCH:
		if (!yochash_mmap(ret, f)) {
	...
	case ETHASH_IO_MEMO_SIZE_MISMATCH:
		...
		if (yochash_io_prepare(dirname, seed_hash, &f, (size_t)full_size, true) != ETHASH_IO_MEMO_MISMATCH) {
	...
	if (!yochash_compute_full_data(ret->data, full_size, light, callback)) {
...
{
	char strbuf[256];
	if (!yochash_get_default_dirname(strbuf, 256)) {
		return NULL;
	}
	uint64_t full_size = yochash_get_datasize(light->block_number);
	yochash_h256_t seedhash = yochash_get_seedhash(light->block_number);
	return yochash_full_new_internal(strbuf, seedhash, full_size, light, callback);
}
