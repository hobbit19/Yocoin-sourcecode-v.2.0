...
// Reference: https://github.com/ethereum/wiki/wiki/Yochash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see yochash_io_prepare
enum yochash_io_rc {
...
/**
 * Logs a critical error in important parts of yochash. Should mostly help
...
/**
 * Prepares io for yochash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the yochash
 ...
 *                           https://github.com/ethereum/wiki/wiki/Yochash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum yochash_io_rc
 */
enum yochash_io_rc yochash_io_prepare(
	char const* dirname,
	yochash_h256_t const seedhash,
	FILE** output_file,
	uint64_t file_size,
	bool force_create
);
 ...
 */
FILE* yochash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* yochash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool yochash_mkdir(char const* dirname);
 ...
 */
bool yochash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* yochash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/ethereum/wiki/wiki/Yochash-DAG
 *
 ...
 */
bool yochash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool yochash_io_mutable_name(
	uint32_t revision,
