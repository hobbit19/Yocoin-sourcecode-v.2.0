...
)
{
	...
	enum yochash_io_rc ret = ETHASH_IO_FAIL;
	// reset errno before io calls
	errno = 0;

	// assert directory exists
	if (!yochash_mkdir(dirname)) {
		ETHASH_CRITICAL("Could not create the yochash directory");
		goto end;
	}

	yochash_io_mutable_name(ETHASH_REVISION, &seedhash, mutable_name);
	char* tmpfile = yochash_io_create_filename(dirname, mutable_name, strlen(mutable_name));
	if (!tmpfile) {
	...
	if (!force_create) {
		...
		f = yochash_fopen(tmpfile, "rb+");
		if (f) {
			size_t found_size;
			if (!yochash_file_size(f, &found_size)) {
				fclose(f);
