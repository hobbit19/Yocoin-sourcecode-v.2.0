...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	proBc, _ := NewBlockChain(proDb, nil, &proConf, yochash.NewFaker(), vm.Config{})
	defer proBc.Stop()

	conDb := yocdb.NewMemDatabase()
	...
	conBc, _ := NewBlockChain(conDb, nil, &conConf, yochash.NewFaker(), vm.Config{})
	defer conBc.Stop()
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		db = yocdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ := NewBlockChain(db, nil, &conConf, yochash.NewFaker(), vm.Config{})
		defer bc.Stop()
		...
		db = yocdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ = NewBlockChain(db, nil, &proConf, yochash.NewFaker(), vm.Config{})
		defer bc.Stop()
