...
// deployYocstats queries the user for various input on deploying an yocstats
// monitoring server, after which it executes it.
func (w *wizard) deployYocstats() {
	...
	infos, err := checkYocstats(client, w.network)
	if err != nil {
		infos = &yocstatsInfos{
			port:   80,
			host:   client.server,
	...
	if out, err := deployYocstats(client, w.network, infos.port, infos.secret, infos.host, trusted, infos.banned, nocache); err != nil {
