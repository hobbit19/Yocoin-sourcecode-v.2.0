...
// makeWizard creates and returns a new puppyoc wizard.
...
// setting up a new or managing an existing YoCoin private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppeth, your YoCoin private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new YoCoin network down to  |")
	...
	log.Info("Administering YoCoin network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppyoc", w.network)
