...
var (
	VipLogFlag = cli.BoolFlag{
		Name:  "viplog",
		Usage: "Extra logging output. V I P only. 👑",
	}
	// General settings
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "YOC", yocPeers, "Light-YOC", lightPeers, "total", cfg.MaxPeers)

	if ctx.GlobalIsSet(MaxPendingPeersFlag.Name) {
		cfg.MaxPendingPeers = ctx.GlobalInt(MaxPendingPeersFlag.Name)
	}
	if !ctx.GlobalIsSet(EnableDiscoverFlag.Name) || lightClient || nov2019.GetNov2019MigrationManager().Is2019RetestNet() {
		cfg.NoDiscovery = true
	}

	if lightPeers < 10 {
		log.Warn("Peers (light) is low", "count", lightPeers)
	}
	if yocPeers < 10 {
		log.Warn("Peers (YOC) is low", "count", yocPeers)
	}

	// if we're running a light client or server, force enable the v5 peer discovery
	// unless it is explicitly disabled with --nodiscover note that explicitly specifying
	// --v5disc overrides --nodiscover, in which case the later only disables v4 discovery
	forceV5Discovery := (lightClient || lightServer) && ctx.GlobalBool(EnableDiscoverFlag.Name) && !nov2019.GetNov2019MigrationManager().Is2019RetestNet()
	...
	if ctx.GlobalBool(DeveloperFlag.Name) || nov2019.GetNov2019MigrationManager().Is2019RetestNet() {
