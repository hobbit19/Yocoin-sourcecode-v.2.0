...
import (
	...
	"github.com/Yocoin15/Yocoin_Sources/swarm/network/stream"
	"github.com/Yocoin15/Yocoin_Sources/swarm/pss"
	"github.com/Yocoin15/Yocoin_Sources/swarm/state"
	"github.com/Yocoin15/Yocoin_Sources/swarm/storage"
	"github.com/Yocoin15/Yocoin_Sources/swarm/storage/mock"
	"github.com/Yocoin15/Yocoin_Sources/swarm/storage/mru"
...
func NewSwarm(config *api.Config, mockStore *mock.NodeStore) (self *Swarm, err error) {
	...
	if config.SwapAPI != "" && config.SwapEnabled {
		...
		backend, err = yocclient.Dial(config.SwapAPI)
		if err != nil {
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config, privkey *ecdsa.PrivateKey) (*ensClient, error) {
	...
	yocClient := yocclient.NewClient(client)
	...
	dns, err := ens.NewENS(transactOpts, ensRoot, yocClient)
	if err != nil {
		return nil, err
	}
