...
func (api *PrivateMinerAPI) Start(threads *int) error {
	...
	if th, ok := api.e.engine.(threaded); ok {
		if *threads < 1 {
			*threads = 4
			log.Info(nov2019.LOG_PREFIX+"zero mining threads was requested; fixed", "nov2019", true)
		}
		log.Info("Updated mining threads", "threads", *threads, "mining", api.e.IsMining(), "yocbase", api.e.miner.GetYocbase())
