...
// a Yocoin console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = yoc.getBlock(yoc.getTransaction(tx).blockHash);
  var genesis = yoc.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.yoc.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  yoc.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(yocdb.NewMemDatabase(), test.Genesis.Alloc)
