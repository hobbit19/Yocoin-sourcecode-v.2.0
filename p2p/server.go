...
func (srv *Server) encHandshakeChecks(peers map[discover.NodeID]*Peer, inboundCount int, c *conn) error {
	switch {
	case !c.is(trustedConn|staticDialedConn) && (len(peers) >= srv.MaxPeers && srv.MaxPeers > 10):
		return DiscTooManyPeers
	case !c.is(trustedConn) && c.is(inboundConn) && inboundCount >= srv.maxInboundConns() && inboundCount > 10:
		return DiscTooManyPeers
	case peers[c.id] != nil:
